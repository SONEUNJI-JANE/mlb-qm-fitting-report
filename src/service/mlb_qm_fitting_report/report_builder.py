...
// dim -> 선택된 값(문자열) | null(전체).
const analysisFilters = {quarter: null, item: null, td: null, qa: null, vendor: null};
...
function resetFiltersIfSeasonChanged(season) {
  ...
  Object.keys(analysisFilters).forEach(dim => { analysisFilters[dim] = null; });
}
...
function filterOptionsHtml(rows, dim) {
  ...
  const opt = v => `<option value="${escSvg(v)}"${cur === v ? ' selected' : ''}>${esc(v)}</option>`;
  const allOpt = `<option value=""${!cur ? ' selected' : ''}>전체</option>`;
  if (dim !== 'vendor') return allOpt + values.map(opt).join('');
  const byCat = {};
  values.forEach(v => { const cat = VENDOR_CATEGORY[v] || '기타'; (byCat[cat] || (byCat[cat] = [])).push(v); });
  return allOpt + ['KNIT', 'WOVEN', 'SWEATER', 'DENIM', '기타'].filter(c => byCat[c])
    .map(cat => `<optgroup label="${esc(cat)}">${byCat[cat].map(opt).join('')}</optgroup>`).join('');
}

// 필터 드롭다운 한 줄(표 위에 바로 붙임 — 따로 떨어진 패널 아님).
function filterRowHtml(rows) {
  return `<div style="display:flex;gap:14px;flex-wrap:wrap;align-items:center;margin-bottom:10px">` +
    Object.keys(FILTER_DIM_LABELS).map(dim =>
      `<span style="font-size:11px;display:inline-flex;align-items:center;gap:4px">` +
      `<label style="font-weight:700;color:#888">${esc(FILTER_DIM_LABELS[dim])}</label>` +
      `<select onchange="analysisFilters['${dim}']=this.value||null;renderAnalysis()">${filterOptionsHtml(rows, dim)}</select></span>`
    ).join('') + `</div>`;
}
...
function applyAnalysisFilters(rows) {
  return rows.filter(r => Object.keys(analysisFilters).every(dim => {
    ...
    return !sel || filterFieldValue(r, dim) === sel;
  }));
}
...
function renderAnalysis() {
  ...
  if (season === '26FW') {
    ...
    secOnTime.innerHTML = `<h3>${periodLabelKr}별 승인율 (FIT/PP/TOP 별도)</h3>` +
      `<p class="sub">주차별 = 그 ${periodLabelKr}에 due였던 것 중 실제 승인일이 due date 이내였던 비율(정시 승인율, 고정값, 나중에 승인돼도 안 바뀜). ` +
      `누적 = "정시"와 무관하게 그냥 지금까지 승인(완료)된 비율의 누적(메인탭 Due Date 기준과 동일 로직). 이 둘은 원래 다른 숫자가 정상.</p>` +
    ...
    stub.innerHTML = `<h3>${periodLabelKr}별 정시 승인율</h3>` + filterRowHtml(allRows) + controlsHtml +
