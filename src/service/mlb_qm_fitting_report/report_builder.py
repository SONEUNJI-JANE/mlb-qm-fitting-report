  ...
  for (const row of rawRows) {
    ...
    WITHIN_STAGE_PIPELINE.forEach(stage => {
      ...
      rounds.forEach((r, i) => {
        ...
        bucket.next[nextLabel] = (bucket.next[nextLabel] || 0) + 1;
      });
    });
  }
  return result;
    ...
    WITHIN_STAGE_PIPELINE.forEach(stage => {
      ...
      statuses.forEach(status => {
        ...
        html += `<tr>` +
          ...
          `<span style="background:#eef1fa;color:#4a65a9;border-radius:4px;padding:1px 4px;font-size:10px;white-space:nowrap">→${esc(nextLabel)}</span></td>` +
