...
.num-th,.num-td{text-align:right;font-variant-numeric:tabular-nums;width:90px}
.num-td.pct{font-weight:700}
.owner-col{width:64px}
.stage-col{width:64px}
  ...
  for (const season of Object.keys(week.progress).sort()) {
    ...
    table.innerHTML = `<thead>
      <tr><th class="owner-col" rowspan="2">담당</th><th class="stage-col" rowspan="2">단계</th>
        <th class="grp-th" colspan="3">총량대비</th><th class="grp-th" colspan="3">기준대비</th>
        ${isLatest ? '<th class="act-col" rowspan="2"></th>' : ''}</tr>
      <tr><th class="num-th">비율</th><th class="num-th">승인</th><th class="num-th">전체</th>
        <th class="num-th">비율</th><th class="num-th">승인</th><th class="num-th">전체</th></tr>
      </thead><tbody></tbody>`;
    ...
    for (const owner of ['TD', 'QA']) {
      for (const stage of Object.keys(week.progress[season][owner] || {})) {
        ...
        row.innerHTML = `<td>${esc(owner)}</td><td>${esc(stage)}</td>` +
          `<td class="num-td pct" id="cell-${key}">${pct(m.total_done, m.total_all)}%</td><td class="num-td">${m.total_done}</td><td class="num-td">${m.total_all}</td>` +
          `<td class="num-td pct">${pct(m.baseline_done, m.baseline_all)}%</td><td class="num-td">${m.baseline_done}</td><td class="num-td">${m.baseline_all}</td>` +
